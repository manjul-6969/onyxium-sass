...
class Chat {
  ...
  async fetchHistory(pageNumber) {
    ...
    if (pageNumber != null && typeof pageNumber != "number") throw Error("Invalid arguments");

    const client = this.client;
    const pageQuery = pageNumber ? `&page_num=${pageNumber}` : "";

    const request = await this.requester.request(
      `https://beta.character.ai/chat/history/msgs/user/?history_external_id=${this.externalId}${pageQuery}`,
      {
  ...
  async sendAndAwaitResponse(optionsOrMessage, singleReply) {
    ...
    if (request.status() === 200) {
      ...
      if (response["force_login"] == true || response["abort"] == true)
        return new Reply(this, response);

      const replies = response.replies;
      const messages = [];

      for (let i = 0; i < replies.length; i++) messages.push(new Reply(this, response, i));
  ...
  async generateImage(prompt) {
    ...
    const request = await this.requester.request(
      "https://beta.character.ai/chat/generate-image/",
      {
        headers: client.getHeaders(),
        method: "POST",
        client: this.client,
  ...
  async changeToConversationId(conversationExternalId, force = false) {
    if (typeof conversationExternalId != "string" || typeof force != "boolean")
  ...
  async deleteMessage(messageId) {
    if (typeof messageId != "string")
      throw Error("Invalid arguments - (Message ids are now strings)");
    if (!this.client.isAuthenticated()) throw Error("You must be authenticated to do this.");
  ...
  async deleteMessagesBulk(amount = 50, descending = false, printSteps = false) {
    ...
    if (pagesToFetch >= 10)
      console.warn(
        "[node_characterai] Warning: deleting this much messages in bulk could take a while or get you rate limited, so make sure you know what you're doing. Also, you can set the argument printSteps to true to show debugging steps.",
      );
    ...
    for (let i = 0; i < pagesToFetch; i++) {
      ...
      if (printSteps == true)
        console.log(`[node_characterai] Bulk delete - Current page: ${999999 - i}`);
    ...
    if (printSteps == true)
      console.log(
        `[node_characterai] Bulk delete - Total messages to delete: ${cumulatedHistory.length}`,
      );

...
class Message {
  ...
  async delete(deletePreviousToo = false) {
    ...
    try {
      ...
      throw Error("Failed to delete message." + error);
    }
  }
...
class Reply {
  constructor(chat, options, index) {
    ...
    if (options.force_login == true)
      throw Error(
        "Too many messages! (this might be because you use a guest account or that characterai has restricted messages for guests.)",
      );
    if (options.abort == true)
      throw Error(
        "Could not get the full reply because it was aborted. This happens often when the output was filtered for violent or explicit content.",
      );
